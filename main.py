...
def recogniseFaces(embeddingDict, groundTruthDcit):
    pass
    ...
    for oneFace in faceBox:
        ...
        for f2 in lfbox:
            ...
            if not isNewFace:
                if f2['isRecognised']:
                    oneFace['isRecognised'] = True
                    oneFace['rgNum'] = f2['rgNum']
                    oneFace['name'] = f2['name']
                    break
                if f2['rgNum'] <= TEST_SUM:
                    oneFace['rgNum'] = f2['rgNum']
                    oneFace['lastName'] = f2['lastName']
                ...
                for dataFileList in listdir(tempFile):
                    ...
                    for dataFile in listdir(fullList):
                        if dataFile[-4:] == '.mpg':
                            ...
                            faceData = processOneVideo(fullFilePath, truthFaces)
                            fullData += faceData
